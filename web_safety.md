...
###应用层和网络层的DoS
最经典的网络层`DoS`就是SYN flood了，它利用了`tcp`协议的设计缺陷，由于`tcp`协议的广泛使用，所以目前想要根治这个漏洞是不可能的。
...
攻击者首先使用大量肉鸡服务器和伪造源ip地址，向服务端发送SYN包希望建立`tcp`连接，服务端就会正常的响应SYN/ACK包，等待客户端响应。由于是攻击程序，攻击客户端并不会去响应这些SYN/ACK包，服务端判断超过一定时间客户端还没响应就会丢弃这个连接。如果这些攻击连接数量巨大，最终我们的服务器就会因为等待和频繁的处理这种半连接而失去对正常请求的响应，从而导致拒绝服务攻击成功。

通常我们会依靠一些硬件的防火墙来减轻这类攻击带来的危害，网络层的`DDoS`攻击防御算法非常复杂，我们本节将讨论应用层的`DoS`攻击。

应用层的`DoS`攻击由于伴随着一定的业务和web服务器的特性，所以攻击更加多样化，并且目前商业的硬件设备很难对其做到有效的防御，不过它的危害绝对不比网络层的`DDoS`低。
...
所以关注应用层的`DoS`我们往往需要从实际业务入手，找到可能被攻击的地方，做针对性的防御。
...
我们看到运行性能相差了整整一倍，所以当我们在处理这类情况的数据时，建议使用`Buffer.concat`来做。
