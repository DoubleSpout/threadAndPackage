...
###应用层和网络层的DoS
最经典的网络层DoS就是SYN flood了，它就是利用了`tcp`协议的设计缺陷，由于`tcp`协议的广泛使用，目前想要根治这个漏洞是不可能的。
...
一般我们会依靠一些硬件的防火墙来减轻这类攻击带来的危害，网络层的DDoS攻击防御算法非常复杂，我们本节将讨论应用层的DoS攻击。

应用层的DoS攻击由于伴随着一些业务和web服务器的特性，而且应用层DoS攻击也更加多样化，所以目前商业的硬件设备很难对其做到很有效的防御，而它的危害甚至不会比网络层的DDoS来的低。

比如黑客在攻陷了几个流量比较大的网站之后，在网页注入如下代码：

    <iframe src="http://attack web site url"></iframe>

这样每个访问这些网站的客户端都成了黑客攻击目标网站的帮手，如果被刷的攻击路径是一些需要大量I/O计算的接口的话，目标网站将会很快失去响应，黑客DoS攻击成功。所以关注应用层的DoS我们往往需要从实际业务入手，找到可能被攻击的地方，做针对性的防御。
