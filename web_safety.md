...
当我们输入正确和恶意代码执行结果区别很大，如果我们恶意代码不断加长，最终将导致服务器拒绝服务，上述这个正则表达式漏洞之处就在于它企图通过使用对分组匹配后再进行`+`符号的匹配，来验证多级域名下的合法的邮箱地址，例如：`abc@aaa.bbb.ccc.gmail.com`这样的字符串。

正则表达式DoS不仅仅局限于Node.js语言，对于任何一门服务端语言都需要面临这个问题，所以在编写正则表达式时我们应该多留一个心眼，必须仔细检查它们是否会被DoS攻击。
