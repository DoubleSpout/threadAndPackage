...
当我们输入正确、正常错误和恶意代码执行结果区别很大，如果我们恶意代码不断加长，最终将导致服务器拒绝服务，上述这个正则表达式漏洞之处就在于它企图通过使用对分组匹配后再进行`+`符号的匹配，来验证多级域名下的合法的邮箱地址，例如：`abc@aaa.bbb.ccc.gmail.com`。

正则表达式的DoS不仅仅局限于Node.js语言，使用任何一门语言进行开发都需要面临这个问题，不过在使用正则来编写`express`框架的路由时尤其需要注意，总之在使用正则表达式时我们应该多留一个心眼，必须仔细检查它们是否足够强壮，避免它被DoS攻击。
